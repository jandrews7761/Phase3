...
class MartaHack:
    ...
    def logIn(self):
        ...
        if c == 0:
            ...
            messagebox.showerror("ARF","MULTIPLE USERS HAVE THESE CREDENTIALS. I HAVE NO IDEA HOW THIS IS POSSIBLE")
        else:
        ...
        if not failed:
            ...
            if self.v.get() == "True":
                ...
                if c2 >= 1:
                    ...
                    #suspend the bcard
                    newOwner = "Suspended"
                    sql = '''INSERT into Conflict VALUES (%s, %s, %s)'''
                    x = strftime("%Y-%m-%d %H:%M:%S", gmtime())
                    self.cursor.execute(sql, (self.usrE.get(), x, self.bzNumE.get()))
                    sql2 = '''UPDATE Breezecard SET BelongsTo = %s WHERE Breezecardnum = %s'''
                    self.cursor.execute(sql2, (newOwner, self.bzNumE.get()))
                    self.db.commit()
                else:
    ...
    def adminHome(self):
        ...
        b2 = Button(self.adminHomeWin,text="Suspended Cards", command= self.suspCardWindow,bg=self.fgColor1)
    ...
    def suspCardWindow(self):
        self.suspCardWin = Toplevel()
        self.suspCardWin.title("Suspended Card Management")
        topF = Frame(self.suspCardWin, bg=self.bgColor1)
        topF.grid(row=0, column=0, pady=15, padx=10)
        sql = '''SELECT Breezecard.BreezecardNum, Conflict.Username as newOwner,Conflict.DateTime, Breezecard.BelongsTo as oldOwner
                              from Conflict INNER JOIN Breezecard ON Conflict.BreezecardNum = Breezecard.BreezecardNum'''
        self.cursor.execute(sql)
        x = list(self.cursor.fetchall())
        self.suspData = x
        data = self.suspData
        header = ["Breezecard Number", "Previous Owner", "DateTime", "New Owner"]
        self.suspCardListBox = MultiColumnListbox(topF, header, data)
        botF = Frame(self.suspCardWin, bg=self.bgColor1)
        botF.grid(row=1, column=0, pady=15, padx=10)
        b1 = Button(botF, text="Assign Card to Previous Owner", bg=self.fgColor1, command=self.assignSuspCardOldOwner)
        b1.grid(row=1, column=1, sticky=NSEW, pady=5, padx=5)
        b2 = Button(botF, text="Assign Card to New Owner", bg=self.fgColor1, command=self.assignSuspCardNewOwner)
        b2.grid(row=1, column=2, sticky=NSEW, pady=5, padx=5)
        self.suspCardTopF = topF

        
    def updateSuspCardListBox(self):
        for item in self.suspCardTopF.grid_slaves():
            item.destroy()
        data = self.suspData
        header = ["Breezecard Number", "Previous Owner", "DateTime", "New Owner"]
        self.suspCardListBox = MultiColumnListbox(topF, header, data)
    ...
    def createNewStation(self):
        ...
        if not failed:
            ...
            self.cursor.execute(sql.format(stopID, stationName, newFare, closedStatus, isTrain))
            if self.v2.get() == "False":
                sql2 = '''INSERT into BusStationIntersection values(%s, %s)'''
                self.cursor.execute(sql2, (stopID, nearestInt))
            self.db.commit()
            self.updateStnMgtListBox()
            self.createStationWin.destroy()
    ...
    def assignSuspCardNewOwner(self):
        x = self.suspCardListBox.gotClicked()[0]
        y = self.suspCardListBox.gotClicked()[3]
        sql = """Update Breezecard
        set BelongsTo = %s where BreezecardNum = %s;"""
        self.cursor.execute(sql, (y,x))
        sql2 = """Delete from Conflict
        where BreezecardNum = %s;
        """
        self.cursor.execute(sql2, (x))
        self.db.commit()
        self.updateSuspCardListBox()

    def assignSuspCardOldOwner(self):
        x = self.suspCardListBox.gotClicked()[0]
        y = self.suspCardListBox.gotClicked()[1]
